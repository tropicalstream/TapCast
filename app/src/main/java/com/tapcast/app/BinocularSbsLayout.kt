...
class BinocularSbsLayout @JvmOverloads constructor(
    ...
    /** Stationary hold on the right pad: open RayNeo's real Control Center. */
    var systemHoldHandler: (() -> Unit)? = null
    ...
    /** Vertical hover steps: -1 = up, +1 = down. When set, replaces the
     *  swipe-up-dismiss behavior (the app routes dismissal itself). */
    ...
    private val systemHoldRunnable = Runnable {
        if (activeSide == Side.RIGHT_CURSOR) {
            systemHoldTriggered = true
            cancelMultiTap()
            stopEdgeScroll()
            android.util.Log.i("TapCastInput", "right-pad hold -> RayNeo Control Center")
            systemHoldHandler?.invoke()
        }
    }
    ...
    /** MainActivity owns temple key sequencing so it sees the long-press repeat
     *  before Android's view focus dispatcher can consume it. */
    fun onTempleTap() {
        pokeCursor()
        onTapUp()
    }
    ...
    private fun handleGlassesInput(event: MotionEvent, logicalWidth: Int): Boolean {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN, MotionEvent.ACTION_HOVER_ENTER -> {
                ...
                systemHoldTriggered = false
                removeCallbacks(systemHoldRunnable)
                ...
                ) postDelayed(systemHoldRunnable, SYSTEM_HOLD_DELAY_MS)
                pokeCursor()
                updateCursor()
                return true
            }
            MotionEvent.ACTION_MOVE, MotionEvent.ACTION_HOVER_MOVE -> {
                ...
                if (abs(localX - downInputX) > touchSlop ||
                    abs(rawY - downInputY) > touchSlop
                ) removeCallbacks(systemHoldRunnable)
                ...
                if (menuNavigationActive?.invoke() == true) {
                    ...
                    // AND a row jump, which read as items being skipped.
                    ...
                    when (menuAxisLock) {
                        AXIS_HORIZONTAL -> if (abs(menuStepAccum) >= MENU_STEP_PX) {
                            ...
                            if (event.eventTime - lastMenuStepMs >= MENU_STEP_COOLDOWN_MS) {
                                android.util.Log.i("TapCastInput", "menu step H ${if (menuStepAccum > 0f) "+1" else "-1"}")
                        ...
                        AXIS_VERTICAL -> if (verticalStepHandler != null) {
                            if (abs(vStepAccum) >= MENU_STEP_V_PX) {
                                if (event.eventTime - lastMenuStepMs >= MENU_STEP_COOLDOWN_MS) {
                                    android.util.Log.i("TapCastInput", "menu step V ${if (vStepAccum > 0f) "+1" else "-1"}")
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL, MotionEvent.ACTION_HOVER_EXIT -> {
                ...
                if (!moved && ended && !held && !systemHoldTriggered) onTapUp()
    ...
    private fun onTapUp() {
        android.util.Log.i("TapCastInput", "tapUp count=${tapCount + 1}")
        ...
        val r = Runnable {
            ...
            val n = tapCount
            tapCount = 0
            android.util.Log.i("TapCastInput", "tap resolve n=$n at ($cursorX,$cursorY)")
            when {
    ...
    private fun performCursorClick() {
        val now = SystemClock.uptimeMillis()
        if (now - lastClickTime < CLICK_DEBOUNCE_MS) { android.util.Log.i("TapCastInput", "click debounced"); return }
        lastClickTime = now
        if (logicalClickHandler?.invoke(cursorX, cursorY) == true) { android.util.Log.i("TapCastInput", "click consumed by handler"); return }
        android.util.Log.i("TapCastInput", "click dispatched at ($cursorX,$cursorY)")
        clickAtCursor()
    }
    ...
    private fun updateEdgeScroll() {
        ...
        if (!edgeScrollActive) {
            ...
            android.util.Log.d("TapCastInput", "edge scroll engaged dy=$edgeScrollDy cursorY=$cursorY")
    ...
    companion object {
        ...
        // Slightly beyond Android/RayNeo's normal long-press boundary. The
        // launcher normally opens Control Center globally; this in-app route
        // guarantees the same official panel even if its monitor was restarted.
